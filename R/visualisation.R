utils::globalVariables(c("country", "gdp"))

#' Creer un graphique des 15 pays avec le plus grand PIB
#'
#' Genere un diagramme en barres horizontal montrant les 15 pays
#' ayant le PIB nominal le plus eleve. Utilise ggplot2 pour la visualisation.
#'
#' @param donnees Un data.frame contenant les colonnes \code{country} et \code{gdp}.
#' @param titre Titre du graphique (par defaut "Top 15 des pays par PIB").
...
#' visualiser_top_pib(world_data, titre = "Economies mondiales")
visualiser_top_pib <- function(donnees, titre = "Top 15 des pays par PIB") {
  ...
  # Recuperer le top 15 et ordonner
  donnees_ordered <- donnees[order(donnees$gdp, decreasing = TRUE), ]
  top_15 <- donnees_ordered[1:15, ]
  top_15 <- top_15[!is.na(top_15$gdp), ]

  if (nrow(top_15) == 0) {
    stop("Aucune donnee valide pour creer le graphique.")
  }

  # Creer le facteur avec les niveaux ordonnes
